...
class Dataframe2SQL:
    """
    Connect between python and Heroku SQL database: create and populate tables
    ...
    def create_table(self, schema: str, table: str) -> None:
        """
    ...
    def populate_table(self, table_name: str, df: pd.DataFrame) -> None:
        """
        ...
        for index, row in df.iterrows():
            self.run_syntax(db_connection=db_connection,
                            syntax=f"INSERT INTO {table_name} VALUES{tuple(row.values)}")
        db_connection.commit()
        db_connection.close()
